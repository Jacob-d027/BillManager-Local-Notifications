...
class BillDetailTableViewController: UITableViewController, UITextFieldDelegate {
    ...
    func presentNeedAuthorizationAlert() {
        let alert = UIAlertController(title: "Authorization Needed", message: "We can't set reminders for you without notification permissions. Please grant us notification permissions if you wish to use reminders.", preferredStyle: .alert)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if remindSwitch.isOn {
            bill.scheduleReminder(setDate: remindDatePicker.date) { updatedBill in
